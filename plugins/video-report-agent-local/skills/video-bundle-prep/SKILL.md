...
This local plugin is bound to the existing project checkout on this Windows machine:

```text
D:\W\Codex\video-summarize-program
```

Run all `uv run video-bundle-agent ...` commands from that project root unless the user explicitly points
to another checkout. Do not assume the plugin directory itself contains the Python `.venv`, outputs, cookies,
MediaCrawler checkout, or whisper.cpp runtime.

Local runtime assumptions:

- MediaCrawler: `D:\W\Codex\external\MediaCrawler`
- whisper.cpp CUDA CLI: `D:\Workshop\whisper.cpp\v1.8.6-cuda\Release\whisper-cli.exe`
- YouTube cookies: `%APPDATA%\video-bundle-agent\youtube.cookies.txt`
- Bilibili cookies: `%APPDATA%\video-bundle-agent\bilibili.cookies.txt`
- Xiaohongshu cookies: `%APPDATA%\video-bundle-agent\xiaohongshu.cookies.txt`
