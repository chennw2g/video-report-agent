...
When the user gives a raw video URL or local video file, first run the `video-bundle-prep` workflow from
that project root, then write/render the report from the generated bundle. Do not treat the plugin directory
as the Python runtime root.

Renderer path in this local plugin checkout:

```text
plugins\video-report-agent-local\skills\video-report\scripts\render_report.py
```

Project renderer path, when running from the project root:
