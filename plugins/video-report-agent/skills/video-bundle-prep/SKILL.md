...
## Release Plugin Runtime

This plugin is a Codex workflow wrapper around a cloned `video-bundle-agent` repository. The plugin
directory itself is not the Python runtime root.

Before running commands, use the active workspace checkout that contains:

```text
pyproject.toml
src/video_bundle_agent/
skills/video-report/scripts/render_report.py
```

Run `uv run video-bundle-agent ...` from that repository root. If the active workspace is not the cloned
project, ask the user for the project path instead of assuming a machine-specific directory.

Portable runtime configuration:

- `VIDEO_BUNDLE_AGENT_TOOL_ROOT` or `VIDEO_REPORT_AGENT_TOOL_ROOT`: optional external tool root.
- `XHS_MEDIACRAWLER_PATH`: optional MediaCrawler checkout path for Xiaohongshu comments. If unset, the
  project first checks `external/MediaCrawler` under the cloned repository.
- Cookie files are never read implicitly. Pass explicit `--cookies` files when a platform requires login.
