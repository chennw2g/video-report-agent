...
When the user gives a raw video URL or local video file, first run the `video-bundle-prep` workflow from the
active workspace checkout that contains `pyproject.toml` and `src/video_bundle_agent/`, then render from the
generated bundle.

Renderer path from the cloned project root:

```text
skills/video-report/scripts/render_report.py
```

If the active workspace is not the cloned project, ask the user for the project path instead of assuming a
