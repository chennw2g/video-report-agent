...
def test_render_report_adds_required_header_metrics(tmp_path: Path) -> None:
    bundle_dir = make_bundle(tmp_path)
    write_json(
        bundle_dir / "metadata.json",
        {
    ...
    for label in ["平台", "作者", "发布时间", "视频时长", "播放量", "评论数", "点赞数", "分享数"]:
        assert label in html
    assert "小红书" in html
    assert "tester" in html
    assert "2026-06-14" in html
    assert "1 分 30 秒" in html
    assert "99" in html
    assert "未获取" in html
    assert "text-align: center;" in html


def test_render_report_localizes_raw_diagnostics(tmp_path: Path) -> None:
    bundle_dir = make_bundle(tmp_path)
    write_json(
        bundle_dir / "diagnostics.json",
        {
            "records": [
                {
                    ...
                    "message": "Client error '404 Not Found' for url 'https://example.test/missing.png'",
    ...
    assert result.returncode == 0, result.stderr
    html = (bundle_dir / "report.zh.deep.html").read_text(encoding="utf-8")
    assert "媒体资源下载失败" in html
    assert "MEDIA_DOWNLOAD_FAILED" not in html
    assert "404 Not Found" not in html
